...
const Threads = ({
  ...
  useEffect(() => {
    ...
    // Only create a new renderer if one doesn't exist
    if (!rendererRef.current) {
      rendererRef.current = new Renderer({ 
        alpha: true,
        premultipliedAlpha: false, // Prevent white flashes
        antialias: true,
    ...
    // Create geometry, program, and mesh if they don't exist
    if (!meshRef.current) {
      ...
      meshRef.current = new Mesh(gl, { geometry, program: programRef.current });
    } else {
    ...
    function resize() {
      ...
      if (clientWidth === 0 || clientHeight === 0) return; // Prevent rendering at 0 size
    ...
    function update(t) {
      if (!isRunning || !rendererRef.current || !meshRef.current || !programRef.current) return;
    ...
    return () => {
      ...
      // Don't destroy the renderer on component update, only on unmount
